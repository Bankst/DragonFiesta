namespace DragonFiesta.Networking.Cryptography
{
    public class FiestaCryptoProvider : ICryptoProvider
    {
        private FiestaCryptoProvider(int pBasePosition)
        {
            PositionInXorKey = pBasePosition;
        }

        public FiestaCryptoProvider(System.UInt16 keyPosition)
        {
            PositionInXorKey = keyPosition;
        }

        public byte[] Encrypt(FiestaPacket pWriter)
        {
            byte[] data = pWriter.GetPacketBytes();
            int offset = 1;
            if (data[0] == 0)
            {
                offset = 3;
            }

            return Encrypt(data, offset, data.Length);
        }

        public byte[] Encrypt(byte[] pData, int pOffset, int pLength)
        {
            for (int i = pOffset; i < pLength; i++)
            {
                if (PositionInXorKey >= CryptionKey.Length)
                {
                    PositionInXorKey = 0;
                }

                pData[i] ^= CryptionKey[PositionInXorKey];
                PositionInXorKey++;
            }
            return pData;
        }

        public byte[] Decrypt(byte[] pData, int pOffset, int pLength)
        {
            return Encrypt(pData, pOffset, pLength);
        }

        public void SetXorKeyPosition(int pKeyPosition)
        {
            PositionInXorKey = pKeyPosition;
        }

        protected int PositionInXorKey;

        #region XorKey

        public static readonly byte[] CryptionKey =
        {
            
        };

        #endregion XorKey
    }
}