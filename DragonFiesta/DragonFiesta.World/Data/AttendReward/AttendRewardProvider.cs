...
namespace DragonFiesta.World.Data.AttendReward
{
    ...
    public class AttendRewardProvider
    {
        ...
        public static void LoadAttendReward()
        {
            ...
            DataLog.WriteProgressBar(">> Load AttendReward");
            using (var mBar = new ProgressBar(pResult.Count))
            {
                for (var i = 0; i < pResult.Count; i++)
                {
                    ...
                    {
                        DataLog.Write(DataLogLevel.Warning, $"Duplicate AR ID found. ID: {info.AR_ID}");
                        continue;
                    }
                ...
                watch.Stop();
                DataLog.WriteProgressBar($">> Loaded {AttendRewardSC.Count} rows from SHN in {(double)watch.ElapsedMilliseconds / 1000}s");
            }
        }
        ...
        public static void LoadAttendSchedule()
        {
            ...
            DataLog.WriteProgressBar(">> Load AttendSchedule");
            ...
            using (var mBar = new ProgressBar(pResult.Count))
            {
                ...
                watch.Stop();
                DataLog.WriteProgressBar($">> Loaded {AttendScheduleSC.Count} rows from SHN in {(double)watch.ElapsedMilliseconds / 1000}s");
            }
        }
    }
}
